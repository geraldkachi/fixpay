// Admin-specific API helpers — Payment Rail module
...
export const railApi = {
  ...
  createRail: (body: {
    tenantId: string; paymentMethod: string; processorId: string
    priority: number; configJson: string
  }) => api.post<PaymentRailConfig>('/admin/rails', body).then(r => r.data),

  updateConfig: (id: string, configJson: string, priority?: number) =>
    api.put<PaymentRailConfig>(`/admin/rails/${id}/config`, { configJson, priority }).then(r => r.data),

  updateProcessor: (id: string, processorId: string) =>
    api.put<PaymentRailConfig>(`/admin/rails/${id}/processor`, { processorId }).then(r => r.data),

  toggleEnabled: (id: string, enabled: boolean) =>
    api.patch<PaymentRailConfig>(`/admin/rails/${id}/enabled`, { enabled }).then(r => r.data),

  setMaintenance: (id: string, maintenance: boolean) =>
    api.patch<PaymentRailConfig>(`/admin/rails/${id}/maintenance`, { maintenance }).then(r => r.data),

  deleteRail: (id: string) => api.delete(`/admin/rails/${id}`),

  listProcessorIds: () => api.get<string[]>('/admin/rails/processors').then(r => r.data),

  getProcessorSchema: (processorId: string) =>
    api.get<ConfigSchema>(`/admin/rails/processors/${processorId}/schema`).then(r => r.data),

  getHealth: () => api.get<ProcessorHealthStatus[]>('/admin/rails/health').then(r => r.data),

  listFees: (railId: string) =>
    api.get<FeeSchedule[]>(`/admin/rails/${railId}/fees`).then(r => r.data),

  addFee: (railId: string, body: {
    feeType: FeeType; fixedFeeKobo: number; percentageFee: number
    capKobo?: number | null; minFeeKobo: number
  ...
  listPlugins: () => api.get<PluginInfo[]>('/admin/plugins').then(r => r.data),
  unloadPlugin: (pluginId: string) => api.delete(`/admin/plugins/${pluginId}`),
  reloadPlugins: () => api.post<{ loaded: number }>('/admin/plugins/reload').then(r => r.data),
